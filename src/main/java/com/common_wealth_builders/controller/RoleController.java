...
public class RoleController {
    ...
    @Operation(
            summary = "Assign roles to user (TECH ADMIN)",
            description = """
            Assigns one or more roles to a user.
            Accessible only by TECH_ADMIN and SUPER_ADMIN.
            Multiple roles can be assigned in a single request.
        """
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Roles assigned successfully"),
            @ApiResponse(responseCode = "400", description = "One or more roles already assigned or invalid"),
            @ApiResponse(responseCode = "403", description = "Forbidden - Requires TECH_ADMIN or SUPER_ADMIN role"),
            @ApiResponse(responseCode = "404", description = "User or role not found")
    })
    @PostMapping("/assign")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'TECH_ADMIN')")
    public ResponseEntity<GenericResponse> assignRoles(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Bulk role assignment details",
                    required = true,
                    content = @Content(
        ...
        log.info(
                "Request received: POST /roles/assign - userId={}, roleIds={}",
                request.getUserId(),
                request.getRoleIds()
        );

        // 🚨 SERVICE METHOD IS NOW BULK-AWARE
        GenericResponse response = roleService.assignRolesToUser(request);

        log.info(
                "Response sent: POST /roles/assign - status={}",
